...
7. Creative surface next: Select-tool node selection and move through undoable sessions **(initial: single-select, drag-move, selection border — done)**; then layer list, multi-select, resize handles, z-order controls and zoom controls; consider suppressing canvas-geometry logging while the settings sheet is open.

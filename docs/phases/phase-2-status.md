...
7. Creative surface next: Select-tool node selection and move through undoable sessions **(done)**; layer list panel **(done)**; then multi-select, resize handles, z-order controls, zoom controls and canvas-geometry log suppression while the settings sheet is open.

...
- **Collapsible/expandable secondary canvas toolbar**: chevron at the row end collapses it to a single 40 px expand handle; collapsed state persists (`canvas_toolbar_toggle`).
...
- Note: canvas geometry evidence changes — without the AppBar the canvas starts at the screen top (the transparent bar overlays it), so compact canvas height grows and `safe_top` becomes the status-bar inset only; a fresh device export will record the new geometry.

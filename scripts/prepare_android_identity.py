...
def write_version_file(android_dir: Path, version: str | None) -> bool:
    """Write a Dart-accessible version constant file into the lib directory."""
    if version is None:
        return False
    version_file = android_dir.parent / "lib" / "version.dart"
    # Also check if there's a src/version.dart or similar
    candidates = [
        android_dir.parent / "lib" / "version.dart",
    ]
    for vf in candidates:
        ...
        if vf.exists():
            existing = vf.read_text(encoding="utf-8")
            if existing.strip() == content.strip():
                return False
        vf.write_text(content, encoding="utf-8")
        print(f"  wrote version.dart -> {version} ({vf})")
        return True
    return False

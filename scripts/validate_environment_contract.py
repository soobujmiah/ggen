#!/usr/bin/env python3
"""Validate GGEN's GitHub-centric hybrid development environment contract."""
...
def main() -> int:
    ...
    if not isinstance(data, dict):
        raise ValueError("environment contract root must be a mapping")
    ...
    if project.get("repository_visibility") != "private":
        raise ValueError("GGEN environment contract requires a private repository")
    ...
    for rule in (
        "computer_quality_mandatory",
        "mobile_friendly_ui_mandatory",
        "manual_operation_required",
        "reference_ui_copying_forbidden",
        "fewer_complete_tools_over_many_demo_tools",
    ):
    ...
    for gate in (
        "exact_numeric_control",
        "undo_redo_transaction",
        "keyboard_mouse_touch_stylus_specification",
        "accessible_alternative_for_every_gesture",
        "tests_documentation_and_evidence",
    ):
        if gate not in gates:
            raise ValueError(f"missing mandatory tool gate: {gate}")

    print(
        "Environment and tool-quality contracts verified: GitHub hybrid brain; "
        "computer-quality capabilities with mandatory mobile-friendly UI."
    )
    return 0
